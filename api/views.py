...
def citiesView(request, pk=None):
    try:
        ...
        return JsonResponse({'msg': 'city not found'}, safe=False, status=404)
...
def divisionsView(request, pk=None):
    try:
        ...
        return JsonResponse({'msg': 'division not found'}, safe=False, status=404)
    except Exception as e:
        return JsonResponse({'error': str(e)}, safe=False, status=500)
...
def teamsView(request, pk=None):
    try:
        ...
        return JsonResponse({'msg': 'team not found'}, safe=False, status=404)
...
def playersView(request, pk=None):
    try:
        ...
        if request.method == 'GET':
            if pk == None:
                result = []
                for p in players.objects.raw(
                    r'''select bp.* , bt.name as team_name
                        from base_players bp
                        inner join base_teams bt on bt.id = bp.team_id '''):

                    result.append({'id': p.id, 'name': p.name, 'age': p.age,
                                   'salary': p.salary, 'yellow_cards': p.yellow_cards, 'red_cards': p.red_cards,
                                   'goals': p.goals, 'played_matches': p.played_matches,
                                   'team': p.team_id, 'team_name': p.team_name})
                serializer = PlayersSerializer(result, many=True)
                return JsonResponse({'data': result}, safe=False, status=200)
            else:
                data = players.objects.get(pk=pk)
                serializer = PlayersSerializer(data, many=False)
                return JsonResponse({'data': serializer.data}, safe=False, status=200)
        ...
        return JsonResponse({'msg': 'player not found'}, safe=False, status=404)
    except Exception as e:
        return JsonResponse({'error': str(e)}, safe=False, status=500)
        ...
        if request.method == 'GET':
            ...
            if serializer.is_valid():
                ...
                if request.data['team1_score'] == request.data['team2_score']:
                    team1 = positions.objects.get(
                        teamId_id=request.data['team1'])
                    team2 = positions.objects.get(
                        teamId_id=request.data['team2'])
